...
window.PIBOARD_QUICKSTART = {
  fr: `
    ...
    <h4>Les clés API : trois cas seulement</h4>
    <p>La quasi-totalité des tuiles fonctionne <b>sans aucune inscription</b>. Trois exceptions, à connaître dès maintenant pour ne pas croire à une panne :</p>
    ...
    <p><b>La recherche de la tuile YouTube</b> demande une clé <b>Google</b> (YouTube Data API v3), gratuite elle aussi, à saisir dans les réglages de cette tuile. Nuance importante : la <b>lecture</b> des vidéos, playlists et chaînes fonctionne <b>sans aucune clé</b> — seule la zone de recherche en a besoin. Vous pouvez donc très bien utiliser la tuile sans jamais en créer une. La marche à suivre pas à pas est dans l'aide de la tuile.</p>
    <p>PiBoard n'embarque aucune de ces clés : elles sont délivrées par compte et ne doivent pas être partagées entre installations sans lien.</p>
  ...
  en: `
    ...
    <h4>API keys: three cases only</h4>
    <p>Nearly every tile works <b>with no sign-up at all</b>. Three exceptions, worth knowing now so you don't mistake them for a fault:</p>
    ...
    <p><b>The YouTube tile's search</b> needs a <b>Google</b> key (YouTube Data API v3), free as well, entered in that tile's settings. An important nuance: <b>playing</b> videos, playlists and channels works <b>with no key at all</b> — only the search box needs one. So you can perfectly well use the tile without ever creating one. The step-by-step procedure is in the tile's help.</p>
    <p>PiBoard ships none of these keys: they are issued per account and must not be shared between unrelated installations.</p>
