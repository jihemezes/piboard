    ...
    /* ================= TUILES / TILES ================= */
    {
      ...
      html: {
        fr: `
          ...
          <p><b>Alarmes</b> : jusqu'à 5 alarmes indépendantes, chacune avec son heure, ses jours (tous les jours, jours ouvrés, ou week-end), son libellé et son son — réutilise le même système d'alerte (flash plein écran + son généré) que le widget Compte à rebours. Un bouton « Arrêter » apparaît directement sur la tuile pendant qu'une alarme sonne ; pas de bouton « Repousser », choix assumé. <b>Les alarmes sonnent toujours à l'heure réelle du système</b>, jamais selon un fuseau horaire affiché à titre de référence.</p>
        ...
        en: `
          ...
          <p><b>Alarms</b>: up to 5 independent alarms, each with its own time, days (every day, weekdays, or weekend), label and sound — reuses the same alert system (full-screen flash + generated sound) as the Countdown widget. A "Stop" button appears right on the tile while an alarm rings; no "Snooze" button, a deliberate choice. <b>Alarms always ring at the system's real time</b>, never according to a time zone shown for reference.</p>
...

    {
      ...
      html: {
        fr: `
          ...
          <p><b>Solution : « Corriger le son muet »</b> (réglage de la tuile, désactivé par défaut). Le flux passe alors par le PiBoard, qui convertit <i>uniquement la piste audio</i> dans un format lisible par le navigateur, en <b>recopiant la vidéo telle quelle sans la réencoder</b> — c'est ce qui rend l'opération abordable : mesuré sur un flux 720p, quelques pour cent d'un cœur seulement. Un réencodage vidéo, lui, serait hors de portée d'un Raspberry Pi.</p>
          <p>Deux contreparties, d'où le choix de laisser l'option désactivée par défaut : le flux transite par le PiBoard au lieu d'aller directement du fournisseur au navigateur, et <b>ffmpeg doit être installé</b> — la commande dépend du système : <code>sudo apt install ffmpeg</code> sur un Raspberry Pi ou un PC Linux, <code>brew install ffmpeg</code> sur macOS, <code>winget install Gyan.FFmpeg</code> sous Windows. PiBoard le cherche automatiquement aux emplacements habituels de chaque système — utile surtout sous Windows, où ffmpeg n'est pas fourni par le système et se retrouve rarement dans le PATH. S'il reste introuvable, la tuile le dit clairement, avec la commande d'installation adaptée à <i>votre</i> système, plutôt que d'échouer sans explication. À n'activer que sur les flux effectivement muets.</p>
        ...
        en: `
          ...
          <p><b>Solution: "Fix silent sound"</b> (a tile setting, off by default). The stream then goes through the PiBoard, which converts <i>only the audio track</i> into a browser-readable format, <b>copying the video as-is without re-encoding it</b> — that's what makes the operation affordable: measured on a 720p stream, just a few percent of one core. A video re-encode, by contrast, would be out of a Raspberry Pi's reach.</p>
          <p>Two trade-offs, hence leaving the option off by default: the stream transits through the PiBoard instead of going straight from the provider to the browser, and <b>ffmpeg must be installed</b> — the command depends on the system: <code>sudo apt install ffmpeg</code> on a Raspberry Pi or Linux PC, <code>brew install ffmpeg</code> on macOS, <code>winget install Gyan.FFmpeg</code> on Windows. PiBoard looks for it automatically in each system's usual locations — particularly useful on Windows, where ffmpeg doesn't ship with the system and rarely ends up in PATH. If it still can't be found, the tile says so clearly, with the install command matching <i>your</i> system, rather than failing without explanation. Only enable it on streams that are actually silent.</p>
