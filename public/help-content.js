    ...
    /* ================= TUILES / TILES ================= */
    {
      ...
      html: {
        ...
        en: `
          ...
          <h4>Possibilities</h4>
          <p>In digital mode, the time fills the available space and automatically recalculates on every tile resize. On a wide, short tile, place the date next to the time rather than below it to make better use of the width. In analog mode, the face automatically sits on the left and the date on the right as soon as the date is shown — the face keeps the tile's full height instead of sharing it with the text below. The date format ranges from full (weekday included) to short (MM/DD/YYYY), handy for freeing up room on a small tile. The name day, a French tradition, can be added next to the date (only when the interface language is French) — its arrangement (below or side by side) automatically switches to "side by side" when the tile is too short for a 2nd line, so it never overflows the tile. The optional day/night background gives an extra visual cue (e.g. a dark gradient at night), automatically following the same solar theme as the rest of the board.</p>
          <p><b>Time zones</b>: a zone (IANA name, e.g. "Asia/Tokyo") can replace the system's for the main displayed time — handy for several Clock tiles, each on a different zone. Up to 3 extra secondary zones show up small in the same tile, for a compact world-clock feel.</p>
