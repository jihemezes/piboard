...

    {
      ...
      html: {
        fr: `
          ...
          <h4>Possibilités</h4>
          <p>La carte se centre sur la ville choisie, indépendamment de la tuile Météo, avec le même choix de fond de carte que les tuiles Trafic et Avions en vue. Deux onglets restent accessibles en un geste directement sur la tuile : <b>Historique</b> (les 2 dernières heures observées, se terminant sur « maintenant ») ou <b>Prévision</b> (uniquement les images de prévision à court terme, quand l'API en fournit — l'onglet n'apparaît que dans ce cas). Les contrôles lecture/pause et image par image restent accessibles directement sur la tuile ; chaque image est préchargée et mise en cache pour une animation fluide, sans rechargement à chaque boucle. La légende (optionnelle) rappelle que les couleurs vont du bleu clair (précipitations légères) au rose (extrême), d'après l'échelle « Universal Blue » de RainViewer — la seule disponible sur l'offre gratuite.</p>
          <h4>Options</h4>
        ...
        en: `
          ...
          <h4>Possibilities</h4>
          <p>The map centers on the chosen city, independent from the Weather tile, with the same choice of base map as the Traffic and Planes Overhead tiles. Two tabs stay one tap away right on the tile: <b>History</b> (the last 2 observed hours, ending on "now") or <b>Forecast</b> (only the short-term forecast frames, when the API provides any — the tab only appears in that case). Play/pause and step controls stay right on the tile; each frame is preloaded and cached for a smooth animation, with no reloading on every loop. The optional legend is a reminder that colors range from light blue (light precipitation) to pink (extreme), based on RainViewer's "Universal Blue" scale — the only one available on the free tier.</p>
          <h4>Options</h4>
