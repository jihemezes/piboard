...

    {
      ...
      html: {
        fr: `
          ...
          <p>La recherche passe par l'API de Google et nécessite <b>votre propre clé API</b>, gratuite : créez un projet sur console.cloud.google.com, activez « YouTube Data API v3 » et créez une clé API, à coller dans les réglages de la tuile. Sans clé, la fenêtre l'explique au lieu de rester vide. Le quota gratuit vaut environ cent pages de résultats par jour — largement assez, mais pas de quoi feuilleter sans fin. La clé est stockée chiffrée sur le serveur et n'est jamais renvoyée au navigateur.</p>
          <h4>Options</h4>
        ...
        en: `
          ...
          <p>Searching goes through Google's API and needs <b>your own API key</b>, free of charge: create a project on console.cloud.google.com, enable "YouTube Data API v3" and create an API key, to paste into the tile's settings. Without a key the window explains that instead of staying empty. The free quota is worth about a hundred result pages a day — plenty, but not endless browsing. The key is stored encrypted on the server and never sent back to the browser.</p>
          <h4>Options</h4>
