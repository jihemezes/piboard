    ...
    },
    {
      ...
      html: {
        ...
        en: `
          ...
          <p>Each tile has its own folder: two Logo tiles do not share an image library. As long as the tile is empty, the <b>"Choose an image"</b> button is shown <b>inside the tile itself</b>: it opens the manager, where images are uploaded, picked or deleted. Once an image is set, that button gives way to the image; to change it, switch to <b>edit mode</b>: a small button appears in the tile's corner, and a second one (a camera) in the crop toolbar. Either reopens the manager. The settings window itself does not let you pick a file: it only shows its name.</p>
