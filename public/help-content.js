...

    {
      ...
      html: {
        fr: `
          ...
          <p>Pour les <b>chaînes en direct</b> spécifiquement, les deux modes utilisent automatiquement <b>VLC</b> pour récupérer le flux quand il est installé : certains fournisseurs IPTV rejettent les requêtes directes de ffmpeg (confirmé par une erreur 405 et par l'examen du code d'un lecteur IPTV de référence, qui utilise nativement le moteur de VLC pour le direct — pas le lecteur du navigateur). Sans VLC installé, les chaînes en direct se replient sur ffmpeg seul, qui reste suffisant pour les fournisseurs sans cette restriction.</p>
<p>Autres contreparties communes aux deux modes : le flux transite par le PiBoard au lieu d'aller directement du fournisseur au navigateur, et <b>ffmpeg doit être installé</b> — la commande dépend du système : <code>sudo apt install ffmpeg</code> sur un Raspberry Pi ou un PC Linux, <code>brew install ffmpeg</code> sur macOS, <code>winget install Gyan.FFmpeg</code> sous Windows. PiBoard le cherche automatiquement aux emplacements habituels de chaque système. Sous Windows, l'installeur de PiBoard propose de l'installer pour vous, en une étape facultative — pratique la plus simple, si vous ne l'avez pas déjà fait. S'il reste introuvable, la tuile le dit clairement, avec la commande d'installation adaptée à <i>votre</i> système, plutôt que d'échouer sans explication.</p>'activer que sur les flux effectivement muets.</p>
        ...
        en: `
          ...
          <p>For <b>live channels</b> specifically, both modes automatically use <b>VLC</b> to fetch the stream when it's installed: some IPTV providers reject direct requests from ffmpeg (confirmed via a 405 error and by examining a reference IPTV player's code, which natively uses VLC's engine for live — not the browser's player). Without VLC installed, live channels fall back to ffmpeg alone, which stays sufficient for providers without this restriction.</p>
