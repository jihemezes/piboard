...
Clock (digital/analog, with an automatic day/night background you can recolor, an optional French name day, and an optional side-by-side time/date layout), weather (with a real photo background matching conditions, an optional French name day, and tomorrow's forecast laid out automatically to fit the tile's shape: side by side, stacked, or hidden), air quality (European Air Quality Index and pollen levels for Europe, compact or fully detailed, no API key), calendar (merge several iCal/.ics calendars, each in its own color, with upcoming-list and week-grid views, recurring events, and iPhone/Google/Nextcloud/Outlook support), weather radar (animated precipitation loop over a real map, powered by RainViewer, no API key), planes overhead (live aircraft near a city on a real map, via a free community ADS-B network — adsb.lol or adsb.fi), notes (Markdown-lite, checklists, sticky-note colors), mailbox (latest messages from an IMAP mailbox, read on tap, nothing stored, strictly read-only), astronomy (moon phase, upcoming visible ISS passes, and visible planets — moon/planets computed locally, no network dependency), RSS/Atom feed (auto-scrolls if there are more articles than fit, tap a linked article for a reader-mode popup with the full article text extracted from the page — ads and navigation stripped out — plus the feed's own illustration, falling back to the feed's summary if extraction fails), web page (iframe), commute time (live driving time with real traffic via TomTom Routing, a comparison to the usual time with a colored alert, and a suggested departure time when you set a desired arrival, for the main route and up to 5 named extra trips), system status (CPU/RAM/disk/temperature of the host), live sports scores, motorsport schedule (full F1/MotoGP race-weekend session timetable) (football and rugby Top 14), league standings (football, rugby, NBA), crypto prices with a tap-to-open 24h/7d/30d/1y chart, quote of the day (offline, real auto-fit text, adjustable 50–200%), countdown / timer (screen flash, beep, and notification webhook on completion), slideshow (upload photos from the tile, a local/NAS folder, a USB key plugged into the Pi, WebDAV, or a URL list — landscape/portrait handled separately, with a customizable border for fully-shown photos), and a live traffic map (official TomTom raster tiles, with a smart refresh schedule), and a network scan (pings the local subnet, cross-checked against the Pi's ARP table, and lists active hosts as "Hostname — IP address"). All can be added, configured, resized and removed from the tile catalog — see [docs/WIDGETS.md](docs/WIDGETS.md) to write your own, or the in-app help (the "?" button) for a full guide to each one.
...
Horloge (digitale/analogique, avec un fond jour/nuit automatique aux couleurs personnalisables, le saint du jour en option, et une disposition heure/date côte à côte en option), météo (avec une vraie photo de fond selon les conditions, le saint du jour en option, et la prévision du lendemain disposée automatiquement selon la forme de la tuile : côte à côte, empilée, ou masquée), qualité de l'air (indice européen et niveaux de pollens pour l'Europe, affichage compact ou entièrement détaillé, sans clé API), agenda (fusion de plusieurs calendriers iCal/.ics, chacun dans sa propre couleur, avec vues liste des prochains événements et grille semaine, événements récurrents, compatible iPhone/Google/Nextcloud/Outlook), radar météo (boucle de précipitations animée sur une vraie carte, via RainViewer, sans clé API), avions en vue (avions en vol en temps réel près d'une ville sur une vraie carte, via un réseau ADS-B communautaire gratuit — adsb.lol ou adsb.fi), bloc-notes (Markdown léger, listes à cocher, couleurs post-it), courriel (derniers messages d'une boîte IMAP, lecture au clic, rien n'est stocké, strictement en lecture seule), astronomie (phase de lune, prochains passages visibles de l'ISS, et planètes visibles — lune/planètes calculées localement, sans dépendance réseau), flux RSS/Atom (défile automatiquement s'il y a plus d'articles que de place, tap sur un article pourvu d'un lien pour une popup façon mode lecture avec le texte complet extrait de la page — pubs et menus écartés — plus l'illustration fournie par le flux, avec repli sur le résumé du flux si l'extraction échoue), page web (iframe), trajet domicile-travail (temps de trajet en direct avec trafic réel via TomTom Routing, comparaison au temps habituel avec alerte colorée, et heure de départ conseillée si une heure d'arrivée souhaitée est renseignée, pour le trajet principal et jusqu'à 5 trajets supplémentaires nommés), état système (CPU/RAM/disque/température de la machine hôte), scores sportifs en direct, sports mécaniques (programme complet des séances d'un week-end F1/MotoGP) (football et rugby Top 14), classement (football, rugby, NBA), cours de cryptos avec courbe 24h/7j/30j/1an au clic, citation du jour (hors-ligne, ajustement automatique du texte, réglable de 50 à 200 %), compte à rebours / minuteur (flash d'écran, bip et webhook de notification à la fin), diaporama (photos téléversées depuis la tuile, dossier local/NAS, clé USB branchée sur le Pi, WebDAV, ou liste d'URLs — formats paysage/portrait traités séparément, avec une bordure personnalisable pour les photos affichées en entier), et une carte de trafic en direct (tuiles raster officielles TomTom, avec un calendrier de rafraîchissement intelligent), et une analyse réseau (ping du sous-réseau local, recoupé avec la table ARP du Pi, et liste des hôtes actifs au format « Nom d'hôte — Adresse IP »). Toutes se rajoutent, se configurent, se redimensionnent et se suppriment depuis le catalogue de tuiles — voir [docs/WIDGETS.md](docs/WIDGETS.md) pour en écrire une nouvelle, ou l'aide intégrée (bouton « ? ») pour un guide complet de chacune.
