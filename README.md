...
- **Quick start guide** — a one-minute guide shown on first launch, covering the toolbar, the difference between global and per-tile settings, resizing, and leaving edit mode. Re-openable at any time from the help, or on every launch if you prefer.
- **Scrollable board** — when a tile no longer fits, the board becomes scrollable and a thin scrollbar appears — and only then. Mouse wheel, or two fingers on a touchscreen, so a one-finger drag stays available to the tiles themselves.
...
- **Colour themes** — over fifty themes (dark, light, colourful, pink, neutral, monochrome, two-tone, and elegant ones matching the library backgrounds), each changing all colours plus tile corners, border width and font. Apply one to the whole board, to a single page (dashboard mode) or a single drawer (classic mode), with a live preview before applying. A theme editor creates your own — optionally starting from an image, computed locally — with a readability guard (WCAG contrast) that blocks unreadable text, plus JSON export/import.
...
The catalog groups them by family — the same grouping you see when adding a tile.

**Weather** — clock (digital/analog, automatic day/night background you can recolor, optional French name day, optional side-by-side time/date), weather (real photo background matching conditions, tomorrow's forecast laid out to fit the tile's shape), air quality (European AQI and pollen, no API key), weather radar (animated precipitation loop over a real map, via RainViewer, no API key), astronomy (moon phase, upcoming visible ISS passes, visible planets — moon and planets computed locally, no network needed).

**Information** — RSS/Atom feed (auto-scrolls, reader-mode popup with the article text extracted from the page), web page (iframe with zoom and periodic reload), crypto prices (tap for a 24h/7d/30d/1y chart), **stocks & indices** (indices, shares, currencies and commodities; pick from a per-exchange list including CAC 40 / DAX / Dow Jones constituents, or type any symbol; market-closed indicator that accounts for each exchange's time zone; tap for a 1-day intraday to 5-year chart).

**Getting around** — live traffic map (official TomTom raster tiles, quota-aware refresh schedule), commute time (live driving time with real traffic, comparison to the usual time, suggested departure for a desired arrival, up to 5 named extra trips), planes overhead (live aircraft on a real map, via a free community ADS-B network).

**Home & energy** — **Tempo colour** (EDF's blue/white/red electricity tariff as a full-tile band readable across the room, tomorrow's colour once published, remaining days per colour; correctly handles the 6am-to-6am Tempo day), **Home Assistant** (read-only view of your entities — room temperatures, open doors, consumption — updated live over WebSocket; calls no service and cannot change anything in your home).
...
**Entertainment** — TV guide (French DTT and Canal+, turnkey), **TV channels** (a plain player for an IPTV `.m3u` playlist — PiBoard provides no channels and no content whatsoever; it only works with a playlist you already have; records the channel you are watching to a file, live or VOD, with scheduled recordings, resumption after a dropped feed and duration/free-space guards), **TV recordings** (what is recording right now and what has been recorded, with stop, convert to .mp4 and delete), **YouTube** (a video, a playlist, a channel's latest videos or a pasted queue, in YouTube's official privacy-enhanced player — no tracking cookies, no personalized ads; full-screen search window with sorting and paging, using your own free API key), slideshow (photos uploaded from the tile, a local/NAS folder, a USB key, WebDAV, or a URL list; landscape and portrait handled separately).

**Sport** — live scores, motorsport schedule (full F1/MotoGP race-weekend session timetable), league standings (football, rugby, NBA).

**System & network** — system status (CPU/RAM/disk/temperature, network adapter IP addresses with a full configuration window — gateway, DHCP, lease, DNS, domain suffix — and live usage charts on tap), **Internet health** (latency, jitter and packet loss measured continuously by the server, 24 h curve with outages flagged, periodic capped speed tests, and on-demand CSV archiving), network scan (pings the local subnet, cross-checked against the ARP table, lists active hosts, with friendly names you can export to and import from a CSV file), **network watch** (checks that the hosts and services you rely on are answering — a NAS, a Raspberry Pi, an Umbrel server, but also remote services such as Alexa or Somfy; ping, TCP port or HTTP, picked automatically from what you type, with the reason and the duration of each failure), **AI usage** (progress bars for your Claude account limits — the 5-hour and 7-day windows — with reset times; percentages come from the provider, nothing is estimated).
...
### Install (Raspberry Pi OS Bookworm or Trixie, Lite or Desktop)
...
**CARTO now requires a free API key for its base maps.** This affects the Traffic map, Weather radar and Planes overhead tiles, which share the same base map. Without a key the maps still work, but CARTO stamps an "API KEY REQUIRED" watermark across every tile image. Request one at [carto.com/basemaps/apikey](https://carto.com/basemaps/apikey/) (free, no account, 5 million tiles per calendar month) and enter it once in the general settings, under "Maps". PiBoard deliberately ships no key of its own: CARTO issues keys per customer and forbids sharing them across unrelated projects, so a key embedded in this repository would be used by every installation at once, exhaust the shared quota, and break everyone's maps the moment it was revoked.
...
Several widgets call free, keyless public APIs: Open-Meteo (weather, geocoding, and air quality/pollen via CAMS Europe — attribution to CAMS and Open-Meteo shown on the tile as required by their terms of use), RainViewer (animated precipitation radar — attribution shown on the tile as required by their free terms of use), adsb.lol / adsb.fi (community ADS-B flight tracking — attribution shown on the tile), OpenStreetMap Nominatim (commute time address geocoding — please keep refresh intervals reasonable, this is a shared public service), TomTom's Routing API (commute time — same API key as the Traffic tile, free tier 2500 requests/day, shared account-wide), CoinGecko (crypto prices), ESPN's public (unofficial) scoreboard/standings endpoints (live sports scores and league tables), Jolpica-F1 (Formula 1 calendar and session times — the community successor to the retired Ergast API) and motogp.com's public feed (MotoGP session times; undocumented, so it may change without notice), astronomy-engine (local moon phase and planet position calculations, MIT-licensed) and iss-api.polluxlabs.io (free, keyless ISS pass predictions; a solo, donation-funded project, same disappearance risk as its retired predecessor open-notify.org), and Openverse (Creative Commons / public domain photo backgrounds for the weather tile — one photo is cached per condition for 7 days, so the anonymous rate limit is never an issue; the photographer's name and license are shown as required by Creative Commons attribution). These are third-party services outside PiBoard's control and may change or become unavailable; each widget degrades to a simple "unavailable" message rather than breaking the board.
...
- **Guide de démarrage rapide** — un guide d'une minute affiché au premier lancement : la barre d'outils, la différence entre réglages généraux et réglages de tuile, le redimensionnement, et comment sortir du mode édition. Réaffichable à tout moment depuis l'aide, ou à chaque lancement si vous préférez.
- **Tableau défilant** — quand une tuile ne tient plus, le tableau devient défilant et un fin ascenseur apparaît — et seulement dans ce cas. Molette, ou deux doigts sur un écran tactile, pour qu'un glissement à un doigt reste disponible aux tuiles elles-mêmes.
...
- **Thèmes de couleurs** — plus de cinquante thèmes (sombres, clairs, très colorés, roses, neutres, monochromes, bichromes, et des thèmes élégants assortis aux fonds de la bibliothèque), qui changent toutes les couleurs ainsi que l'arrondi des tuiles, l'épaisseur des bordures et la police. Applicables à tout le board, à une seule page (mode tableau de bord) ou à un seul volet (mode classique), avec un aperçu en direct avant d'appliquer. Un éditeur crée vos propres thèmes — au besoin à partir d'une image, calculé sur place — avec un garde-fou de lisibilité (contraste WCAG) qui bloque les textes illisibles, et l'export/import en JSON.
...
Le catalogue les regroupe par famille — le même classement qu'à l'ajout d'une tuile.

**Météo** — horloge (digitale/analogique, fond jour/nuit automatique aux couleurs personnalisables, saint du jour en option, disposition heure/date côte à côte en option), météo (vraie photo de fond selon les conditions, prévision du lendemain disposée selon la forme de la tuile), qualité de l'air (indice européen et pollens, sans clé API), radar météo (boucle de précipitations animée sur une vraie carte, via RainViewer, sans clé API), astronomie (phase de lune, prochains passages visibles de l'ISS, planètes visibles — lune et planètes calculées localement, sans réseau).

**Informations** — flux RSS/Atom (défilement automatique, popup façon mode lecture avec le texte de l'article extrait de la page), page web (iframe avec zoom et rechargement périodique), cours cryptos (courbe 24h/7j/30j/1an au clic), **bourse et indices** (indices, actions, devises et matières premières ; choix dans une liste par place, compositions du CAC 40 / DAX / Dow Jones incluses, ou saisie libre de n'importe quel symbole ; indicateur de marché fermé tenant compte du fuseau de chaque place ; courbe du jour en intrajournalier jusqu'à 5 ans au clic).

**Déplacements** — carte de trafic en direct (tuiles raster officielles TomTom, rafraîchissement économe en quota), trajet domicile-travail (temps de trajet en direct avec trafic réel, comparaison au temps habituel, heure de départ conseillée pour une arrivée souhaitée, jusqu'à 5 trajets supplémentaires nommés), avions en vue (avions en temps réel sur une vraie carte, via un réseau ADS-B communautaire gratuit).
...
**Divertissement** — programme TV (TNT française et Canal+, clé en main), **chaînes TV** (simple lecteur pour une playlist IPTV `.m3u` — PiBoard ne fournit AUCUNE chaîne ni aucun contenu ; la tuile ne fonctionne qu'avec une playlist dont vous disposez déjà ; enregistre dans un fichier la chaîne que vous regardez, en direct comme en VOD, avec enregistrements programmés, reprise après coupure du flux et garde-fous de durée et d'espace disque), **enregistrements TV** (ce qui enregistre et ce qui a été enregistré, avec arrêt, conversion en .mp4 et suppression), **YouTube** (une vidéo, une playlist, les dernières vidéos d'une chaîne ou une file collée, dans le lecteur officiel de YouTube en mode confidentialité avancée — sans cookies de suivi ni publicité personnalisée ; fenêtre de recherche plein écran avec tri et pagination, avec votre propre clé API gratuite), diaporama (photos téléversées depuis la tuile, dossier local/NAS, clé USB, WebDAV, ou liste d'URLs ; paysage et portrait traités séparément).
...
### Installation (Raspberry Pi OS Bookworm ou Trixie, Lite ou Desktop)
...
**CARTO exige désormais une clé API gratuite pour ses fonds de carte.** Cela concerne les tuiles Carte de trafic, Radar météo et Avions en vue, qui partagent le même fond. Sans clé, les cartes fonctionnent toujours, mais CARTO barre chaque tuile d'image d'un filigrane « API KEY REQUIRED ». Demandez-en une sur [carto.com/basemaps/apikey](https://carto.com/basemaps/apikey/) (gratuite, sans compte, 5 millions de tuiles par mois civil) et saisissez-la une seule fois dans les réglages généraux, section « Cartes ». PiBoard n'embarque délibérément aucune clé : CARTO les délivre par client et interdit de les partager entre projets sans lien, si bien qu'une clé placée dans ce dépôt serait utilisée par toutes les installations à la fois, épuiserait le quota commun et casserait les cartes de tout le monde dès sa révocation.
...
Plusieurs tuiles interrogent des API publiques gratuites et sans clé : Open-Meteo (météo, géocodage, et qualité de l'air/pollens via CAMS Europe — attribution à CAMS et Open-Meteo affichée sur la tuile comme l'exigent leurs conditions d'usage), RainViewer (radar de précipitations animé — attribution affichée sur la tuile comme l'exigent leurs conditions d'usage gratuites), adsb.lol / adsb.fi (suivi de vol ADS-B communautaire — attribution affichée sur la tuile), OpenStreetMap Nominatim (géocodage des adresses du trajet domicile-travail — merci de garder des intervalles de rafraîchissement raisonnables, c'est un service public partagé), l'API Routing de TomTom (trajet domicile-travail — même clé API que la tuile Trafic, offre gratuite de 2500 requêtes/jour, partagée au niveau du compte), CoinGecko (cours de cryptos), les points d'accès (non officiels) d'ESPN pour les scores et classements sportifs, Jolpica-F1 (calendrier et horaires des séances de Formule 1 — successeur communautaire de l'API Ergast, retirée) et le flux public de motogp.com (horaires des séances MotoGP ; non documenté, il peut donc changer sans préavis), astronomy-engine (calcul local de la phase de lune et de la position des planètes, licence MIT) et iss-api.polluxlabs.io (prédictions de passage ISS gratuites et sans clé ; projet individuel financé par des dons, même risque de disparition que son prédécesseur retiré open-notify.org), et Openverse (photos de fond Creative Commons / domaine public pour la tuile météo — une photo est mise en cache par condition pendant 7 jours, la limite anonyme n'est donc jamais un problème ; le nom du photographe et la licence sont affichés comme l'exigent les licences Creative Commons). Ce sont des services tiers hors du contrôle de PiBoard, qui peuvent évoluer ou devenir indisponibles ; chaque tuile se replie sur un simple message d'indisponibilité plutôt que de casser le tableau.
