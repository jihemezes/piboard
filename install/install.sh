...
if [[ -d /etc/polkit-1/rules.d ]]; then
  cat > /etc/polkit-1/rules.d/50-piboard-poweroff.rules <<POLKIT
// Genere par install/install.sh de PiBoard. Autorise l'utilisateur du
// service a eteindre la machine, et rien d'autre.
// Generated by PiBoard's install/install.sh. Allows the service user to
// power the machine off, and nothing else.
...
POLKIT
  chmod 644 /etc/polkit-1/rules.d/50-piboard-poweroff.rules
  systemctl reload polkit 2>/dev/null || systemctl restart polkit 2>/dev/null || true
  echo "    OK (/etc/polkit-1/rules.d/50-piboard-poweroff.rules)"
else
  echo "    polkit absent : l'extinction depuis PiBoard ne sera pas disponible."
  echo "    polkit missing: powering off from PiBoard will not be available."
fi
