...
# ---------- 2 bis. Droit d'eteindre la machine ----------
# Le menu "Quitter" de PiBoard propose d'eteindre le Pi. Sans cette
# regle, le service n'en a pas le droit : il tourne sous un utilisateur
# sans privileges, et il ne PEUT PAS passer par sudo -- l'unite declare
# NoNewPrivileges=true, ce qui neutralise les binaires setuid comme
# sudo. Une regle sudoers serait donc sans effet ici.
#
# La voie qui fonctionne est systemd-logind, joint par D-Bus (aucun
# setuid en jeu), dont polkit garde l'entree. On autorise UNE action et
# une seule, power-off, et pour le seul utilisateur du service. Le
# redemarrage, la mise en veille, l'extinction avec d'autres sessions
# ouvertes : rien de tout cela n'est accorde.
#
# Regle en JavaScript (polkit >= 0.106, ce qu'ont Pi OS Bookworm/Trixie,
# Debian 12+ et Ubuntu 22.04+). Sur un systeme plus ancien, le fichier
# est simplement ignore et l'extinction depuis PiBoard echouera
# proprement, avec un message explicatif dans l'interface.
#
# PiBoard's "Exit" menu offers to power the Pi off. Without this rule
# the service is not allowed to: it runs as an unprivileged user, and it
# CANNOT go through sudo -- the unit declares NoNewPrivileges=true,
# which neutralizes setuid binaries such as sudo. A sudoers rule would
# therefore have no effect here.
#
# The route that works is systemd-logind, reached over D-Bus (no setuid
# involved), whose door polkit guards. We allow ONE action and one only,
# power-off, and for the service user alone. Rebooting, suspending,
# powering off with other sessions open: none of that is granted.
echo "==> Droit d'eteindre la machine depuis PiBoard / right to power off from PiBoard"
# La regle est posee par un script dedie, relancable seul sur un Pi
# installe avant la 1.104.0 (les mises a jour ne touchent pas /etc).
# The rule is set by a dedicated script, runnable on its own on a Pi
# installed before 1.104.0 (updates do not touch /etc).
bash "$APP_DIR/install/enable-poweroff.sh" "$RUN_USER" || true
